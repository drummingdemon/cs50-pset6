...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // we remove the trailing CRLF by copying
            // the string to a smaller array
            int stackSize = needle - haystack;
            char lineCut[stackSize + 1];
            strncpy(lineCut, haystack, stackSize);
            
            // not forgetting to add the terminator
            lineCut[stackSize] = '\0';

            // getting rid of spaces and chopping up request
            char* request[3] = { "", "", "" };
            char* token = "";
            int tokenCounter = 0;
            
            // let's get the first token
            token = strtok(lineCut, " ");
            
            // gather all the remaining tokens
            while (token != NULL)
            {
                request[tokenCounter] = token;
                tokenCounter++;
                token = strtok(NULL, " ");
            }
            ...
            // check request-target's beginning slash
            else if (request[1][0] != '/')
            {
                error(501);
                continue;
            ...
            // check if request-target contains double quotes
            else if (strchr(request[1], '\"') != NULL)
            {
                error(400);
                continue;
            }
            ...
            // get the terminator location
            char* end = strchr(request[1], '\0');
            
            // get the question mark location (or null if not exists)
            char* qmark = strchrnul(request[1], '?');
            
            // calculate the length of the query
            int queryLength = end - qmark;
            
            // if the query exists, we allocate the necessary size for it
            // otherwise we give it a size of 1 (shorthand condition is awesome)
            char query[(queryLength < 1) ? 1 : queryLength];
            
            // prepare the copy source of the query
            char* src;
            
            // if we don't have a query, than we do not alter the dest size
            // otherwise we add one to store the terminator
            if (strcmp(qmark, "\0") == 0)
            {
                src = qmark;
            }
            ...
            // as memcpy requires the size to be copied, we check the length
            // of the query and set the copy amount as needed
            size_t copySize;
            
            if (queryLength > 1)
            {
                copySize = queryLength;
            }
            else
            {
                copySize = 1;
            ...
            memcpy(query, src, copySize);
            
            // let's get the absolute path
            char absPath[qmark - request[1] + 1];
            
            // compute the absolute path string length
            int absLength = qmark - request[1];
            
            // do the copying
            strncpy(absPath, request[1], absLength);
            
            // adding terminator to the end
            absPath[absLength] = '\0';
            
            // if the absolute path does not have a dot,
            // then surely the extension is missing
            char* dot = strchr(absPath, '.');
            if (dot == NULL)
            {
                error(501);
                continue;
            ...
            strncat(path, root, strlen(root));
            strncat(path, absPath, strlen(root));
            ...
            // init an array big enough for the extension
            char extension[end - dot];
            
            // copy from the dot pointer until the end of absPath
            memcpy(extension, dot + 1, end - dot);
